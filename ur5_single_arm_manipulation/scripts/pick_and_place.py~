...
class Pick_Place:
    def __init__(self):
        ...
        if not self._grasps_ac.wait_for_server(rospy.Duration(2.0)):
        ...
        if not self._pickup_ac.wait_for_server(rospy.Duration(2.0)):
        ...
        if not self._place_ac.wait_for_server(rospy.Duration(2.0)):
    ...
    # Define target place pose:
        ...
        self._pose_place.position.x = self._pose_coke_can2.position.x-0.5
        self._pose_place.position.y = self._pose_coke_can2.position.y-0.85
        self._pose_place.position.z = self._pose_coke_can2.position.z-0.3
        self._pose_place.orientation = Quaternion(*quaternion_from_euler(0.0, 0.0, 0.0))

        self._pose_coke_can2.position.z = self._pose_coke_can2.position.z - 0.9 # base_link is 0.9 above
        self._pose_place.position.z = self._pose_place.position.z + 0.05 # target pose a little above
        ...
        # Pick Coke can object:
        while not self._pickup(self._arm_group, self._grasp_object2_name, self._grasp_object_width):
            rospy.logwarn('Pick up failed! Retrying ...')
            rospy.sleep(1.0)
        ...
        # Place Coke can object on another place on the support surface (table):
        while not self._place(self._arm_group, self._grasp_object2_name, self._pose_place):
            rospy.logwarn('Place failed! Retrying ...')
            rospy.sleep(1.0)
